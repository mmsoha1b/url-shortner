...
- That ID is encoded to a base62 string (characters `0-9`, `a-z`, `A-Z`) to form the short code returned to the user.
...
- **Locked into base62** — because the short code is never stored, the encoding scheme cannot be changed after URLs have been issued. Any change to the encoding would break all existing short links.
